...
class AstrataSupervisor:
    ...
    def reconcile(self) -> dict[str, Any]:
        hygiene = self._reconcile_runtime_hygiene()
        state = self._load_state()
    ...
    def _service_status(self, service: SupervisedService, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if not running and adopted_pid is not None:
            pid = adopted_pid
        state_adopted = bool(service_state.get("adopted")) and running
        external_adopted = adopted_pid is not None or (bool(health_ok) and not running)
        return {
            "service_id": service.service_id,
            "running": running or adopted_pid is not None or bool(health_ok),
            "owned": running and not bool(service_state.get("adopted")),
            "adopted": state_adopted or external_adopted,
            "pid": pid,
            "health_ok": health_ok,
    ...
    def _reconcile_runtime_hygiene(self) -> dict[str, Any]:
        try:
            db = AstrataDatabase(self.settings.paths.data_dir / "astrata.db")
            db.initialize()
            return reconcile_running_attempts(db)
        except Exception as exc:
            return {"status": "failed", "error": str(exc)}
