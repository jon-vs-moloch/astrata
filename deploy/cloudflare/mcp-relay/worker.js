...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/oauth/register") {
      const payload = await request.json();
      return json(await handleOAuthRegister(payload, env));
    }
...
function gptActionsOpenApi(url, env) {
  ...
  return {
    ...
    info: {
      ...
      version: "0.1.0",
      description:
        "Stable Custom GPT action surface for Astrata. Development-only until Astrata Web provides real account auth and per-user relay routing.",
    },
    servers: [{ url: baseUrl }],
    paths: {
      "/gpt/help": {
...
function oauthChallenge(url) {
  return json(
    ...
    401,
    {
      "www-authenticate": `Bearer resource_metadata="${url.origin}/.well-known/oauth-protected-resource", scope="astrata:read astrata:write"`,
    },
  );
}
...
function oauthProtectedResourceMetadata(url) {
  return {
    ...
    scopes_supported: ["astrata:read", "astrata:write"],
...
function oauthAuthorizationPage(url, error = "", details = {}) {
  ...
  if (!params.scope) params.scope = "astrata:read astrata:write";
...
<html lang="en">
  ...
  <body>
    <main>
      ...
      <p class="disclaimer">Requested scope: <code>${escapeHtml(params.scope || "astrata:read astrata:write")}</code></p>
    </main>
  </body>
</html>`);
}
...
async function handleOAuthAuthorize(formData, env) {
  ...
  const scope = String(formData.get("scope") || "astrata:read astrata:write");
...
async function setOAuthCode(env, code, payload) {
  if (env.astrata_auth_db) {
    ...
    `).bind(
      ...
      payload.resource || "",
      payload.scope || "astrata:read astrata:write",
      payload.expires_at,
      payload.created_at,
      payload.created_at
    ).run();
